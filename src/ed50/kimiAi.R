...
# Calculate the ED90 estimation using Dixon-Mood method
n <- tmp3
n
N <- sum(n)
N
...
se <- ed50_result$'Standard Error of Estimate'

# 计算ED90
# Dixon-Mood方法中，ED90的计算公式为：ED90 = ED50 + G * SE
# 其中G的值取决于剂量步长与ED50估计标准误差的比率
# 我们可以使用gTableOrigin数据表来查找或插值获得G的值
ratio <- diff(dose) / se
g_value <- gTableOrigin[which.min(abs(gTableOrigin$Ratio - ratio)), "G1"]  # 假设ED90落在剂量水平上
# g_value <- gTableOrigin[which.min(abs(gTableOrigin$Ratio - ratio)), "G2"]  # 假设ED90落在两个剂量水平之间
...
ed90_result <- list('Method of Estimation'= 'Dixon-Mood',
                    ...
                    'Confidence Level' = ed50_result$'Confidence Level',
                    'Lower Bound' = ed50_result$'Lower Bound',
                    'Upper Bound' = ed50_result$'Upper Bound')
ed90_result
